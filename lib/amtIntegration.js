...
function isAmtConfigured() {
  return AMT_CONFIG.accessKeyId && AMT_CONFIG.secretAccessKey;
}

// Initialize AMT client (when AWS SDK is available)
...
async function getHitInfo(hitId) {
  ...
  try {
    ...
    /*
    const result = await client.getHIT({ HITId: hitId }).promise();
    return result.HIT;
    */
    return null;
  } catch (error) {
...
async function expireHit(hitId) {
  ...
  try {
    // TODO: Implement when AWS SDK is available
    /*
    await client.updateExpirationForHIT({
      HITId: hitId,
...
async function deleteHit(hitId) {
  ...
  try {
    // TODO: Implement when AWS SDK is available
    /*
    await client.deleteHIT({ HITId: hitId }).promise();
